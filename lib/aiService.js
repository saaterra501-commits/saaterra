...
async function callLLM(messages, systemInstruction = '', jsonMode = false) {
  if (GROQ_API_KEY) {
    const candidateModels = ['groq/compound-mini', 'qwen/qwen3.8-27b', 'groq/compound'];
...
Provide your personalized recommendation, strategy, and 5-year ROI breakdown:`;
  ...
  if (aiResponse) {
    ...
    if (slugMatch) {
      ...
      // Find any active deals mentioned in AI reply
      for (const d of activeDeals) {
        if (d.title && aiResponse.toLowerCase().includes(d.title.toLowerCase())) {
          matchedSlugs.push(d.slug);
        } else if (d.slug && aiResponse.toLowerCase().includes(d.slug.toLowerCase())) {
          matchedSlugs.push(d.slug);
        }
      }
  ...
  // If no slugs parsed, pick top 2 relevant deals for the cards, but DO NOT overwrite AI's response if it succeeded!
  if (matchedSlugs.length === 0) {
    ...
    matchedSlugs = scoredDeals.slice(0, 2).map((item) => item.deal.slug);
  }

  // Only use hardcoded template if AI call completely failed
  if (!finalReply) {
    const topMatches = activeDeals.filter((d) => matchedSlugs.includes(d.slug)).slice(0, 2);
